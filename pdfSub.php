...
class myPDF extends FPDF{
    ...
    function header(){
        ...
        $this->Cell(200,5,'SUB CATEGORIES',0,0,'C');
        $this->Ln(5);
    ...
    function headerTable(){
        ...
        $this->Cell(40,8,'Sub Category',1,0,'C');
    ...
    function viewTable($adb){
        $number = 1;
        $this->SetFont('Times','',12);
        $Query = "SELECT * FROM sub_category ORDER BY sub_cat ASC";
        $statement = $adb->prepare($Query);
        $statement->execute();
        while($data = $statement->fetch(PDO::FETCH_OBJ)){
            $pdf = new myPDF();
            $sub_name = $data->sub_cat;
            $date_created = $data->created_date;
            $date_updated = $data->updated_date;
...
$pdf->headerTable();
$pdf->viewTable($adb);
$pdf->Output("","EMS_sub_categories.pdf","D");
$pdf->Output();
