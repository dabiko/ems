...
if (isset($_GET['payment_method'])){
    ...
    $whereSQL = $orderSQL = '';
    $keywords = $_GET['option'];

    if (!empty($keywords)) {
        $whereSQL = "WHERE p_method LIKE '%".$keywords."%'";
    }
    if (!empty($sortBy)) {
...
}elseif (isset($_GET['option']) && $_GET['option'] === "balance"){
    $searchData = $_GET['option'];
        $option = 0;
    ...
    if (!empty($keywords)) {
        $whereSQL = "WHERE $searchData > $option";
    }
    if (!empty($sortBy)) {
        $orderSQL = " ORDER BY in_id " . $sortBy;
    } else {
...
/** Balance checking ends here */
...
}elseif (isset($_GET['option']) && $_GET['option'] === "pending"){
    ...
    $column = "print";
    $searchVal = 0;
    $stmt = $adb->prepare("SELECT COUNT(*) as InvoiceNum FROM invoice WHERE print=:print");
    $stmt->execute(array('print' => 0));
    $resultNum = $stmt->fetch();
    $rowCount = $resultNum['InvoiceNum'];
    ...
    //get rows
    $stmt = $adb->prepare("SELECT * FROM invoice WHERE print=:print LIMIT $start,$limit");
    $stmt->execute(array('print' => 0));
    $Count = $stmt->rowCount();
    ...
    print $output;
/** Pending search ends here */

}elseif (isset($_GET['option']) && $_GET['option'] === "printed"){
    ...
    if (!empty($keywords)) {
        $whereSQL = "WHERE print LIKE '%" . $keywords . "%'";
    }
    if (!empty($sortBy)) {
        $orderSQL = " ORDER BY in_id " . $sortBy;
    } else {
...
/** Printed Documents ends here */

}elseif (isset($_GET['option']) && $_GET['option'] === "needs_printing"){
    ...
    if (!empty($keywords)) {
        $whereSQL = "WHERE balance=$keywords && print=$keywords";
    }
    if (!empty($sortBy)) {
        $orderSQL = " ORDER BY in_id " . $sortBy;
    } else {
       ...
       $Query = "SELECT COUNT(*) as InvoiceNum FROM invoice WHERE balance=:balance && print=:print";
       $stmt = $adb->prepare($Query);
       $stmt->execute(array(':balance' => 0, ':print' => 0));
       $resultNum = $stmt->fetch();
       $rowCount = $resultNum['InvoiceNum'];
