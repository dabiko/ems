...
/** Defining Class for Pagination */
        ...
        if($pages>1) {
            ...
            if($_GET["page"] == 1)
                $output = $output . '<li style="cursor:not-allowed;color: #bccfd8;" class="page-item disabled"><a class="page-link">First</a></li>
                               ...
                               $output = $output . '<li class="page-item"><a onclick="searchResults(\'' . $href . (1) . '\')" class="page-link">First</a></li>
                                      <li class="page-item"> <a onclick="searchResults(\'' . $href . ($_GET["page"]-1) . '\')" class="page-link" aria-label="Previous">
                                       <span aria-hidden="true">&laquo;</span><span class="sr-only">Previous</span>
                                        </a>
                                    </li>';
            ...
            if(($_GET["page"]-3)>0) {
                if($_GET["page"] == 1)
                    ...
                    $output = $output . '<li class="page-item"><a onclick="searchResults(\'' . $href . '1\')" class="page-link">1</a></li>';
            }
            if(($_GET["page"]-3)>1) {
            ...
            for($i=($_GET["page"]-2); $i<=($_GET["page"]+2); $i++)	{
                ...
                if($_GET["page"] == $i)
                    ...
                    $output = $output . '<li class="page-item"><a onclick="searchResults(\'' . $href . $i . '\')"  class="page-link">'.$i.'</a></li>';
            ...
            if(($pages-($_GET["page"]+2))>0) {
                ...
                $output = $output . '<li class="page-item"><a onclick="searchResults(\'' . $href .  ($pages) .'\')"  class="page-link">' . ($pages) .'</a></li>';
            }

            if($_GET["page"] < $pages)
                             $output = $output . '<li class="page-item"> <a onclick="searchResults(\'' . $href . ($_GET["page"]+1) . '\')" class="page-link" aria-label="Previous">
                                       <span aria-hidden="true">&raquo;</span><span class="sr-only">Next</span>
                                        </a>
                                    </li>
                                    <li class="page-item"><a onclick="searchResults(\'' . $href . ($pages) . '\')" class="page-link">Last</a></li>';
            else
    ...
    function getPrevNext($count,$href) {
        ...
        if($pages>1) {
            ...
            $output = $output . '<li class="page-item"><a onclick="searchResults(\'' . $href . ($_GET["page"]-1) . '\')" class="page-link">Prev</a></li>';
            if($_GET["page"] < $pages)
            $output = $output . '<li class="page-item"><a onclick="searchResults(\'' . $href . ($_GET["page"]+1) . '\')" class="page-link">Next</a></li>';
            else
